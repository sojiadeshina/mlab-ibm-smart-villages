...
def main(text):
		if len(text) == 0:
			watson_response = get_root()
			return print_text(watson_response)
		else:
			watson_response = get_response(text)
			return print_text(watson_response)
			# if get_node(watson_response) == 'HEALTH':
			# 	return print_text(watson_response)
			# elif get_node(watson_response) == 'DOMESTIC':
			# 	main = domestic_abuse
			# 	return print_text()
			# elif get_node(watson_response) == 'ANYTHING_ELSE':
			# 	print_text(response)
			# 	return print_text(response)
			# elif get_node(watson_response) == 'YES':
			# 	print_text
